...
void print_bucket_info(const std::unordered_map<std::string, std::string>& map) {
    std::cout << "Size: " << map.size() << "\n";
    std::cout << "Bucket count: " << map.bucket_count() << "\n";
    std::cout << "Load factor: " << std::fixed << std::setprecision(2) << map.load_factor() << "\n";
...
int main() {
    ...
    std::vector<std::pair<std::string, std::string>> merchants = {
        {"MID_100001", "AMAZON_WEB_SERVICES"},
        {"MID_200045", "WALMART_STORE_3291"},
        {"MID_300078", "SHELL_GAS_STATION"},
        {"MID_400012", "STARBUCKS_CORP"},
        {"MID_500099", "UNITED_AIRLINES"},
        {"MID_600023", "TARGET_STORE_1482"}
    };
